...
public class ContextConverter extends SimpleConverter<Context>
{
    ...
    {
        return new StringNode(object.getValue().isEmpty() ? object.getKey() : object.getKey() + "|" + object.getValue());
    }

    @Override
    public Context fromNode(Node node) throws ConversionException
    {
        String token = node.asString();
        ...
        if (token.contains("|"))
        {
            ...
            {
                throw ConversionException.of(this, node, "Unknown context: {}");
            }
            checkToken = parts[1];
        }
